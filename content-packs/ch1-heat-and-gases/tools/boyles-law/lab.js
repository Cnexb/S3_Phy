...
(function () {
  ...
  function drawChamber() {
    var box = chamberBox();
    var P = pressureKPa();
    var heat = Math.min(1, Math.max(0, (Tc - 20) / 180));
    var art = n1();
    var dens = Math.min(1, particles.length / 70);

    if (art) art.fillStage(ctx, cssW, cssH);
    else {
    ...
    ctx.font = art ? art.font(800, 13) : "800 13px Plus Jakarta Sans, sans-serif";
    ctx.textAlign = "left";
    ctx.textBaseline = "alphabetic";
    ctx.fillText(modeLabel(), 20, 28);

    if (art) art.drawGlassChamber(ctx, box.x, box.y, box.w, box.h, heat, dens);
    else {
    ...
    if (mode === "boyle" || mode === "charles") {
      if (art) art.drawPiston(ctx, box.x + box.w, box.y, box.h, "V");
      else {
        ctx.fillStyle = "#475569";
        roundRect(ctx, box.x + box.w, box.y - 6, 14, box.h + 12, 4);
        ctx.fill();
      }
    }

    if (art) art.drawHeatBar(ctx, box.x + 12, box.y + box.h + 14, Math.max(48, box.w - 24), heat);

    drawGauge(box.x + box.w * 0.5, box.y - 54, P);

    for (var i = 0; i < particles.length; i++) {
      var p = particles[i];
      if (art) art.drawDot(ctx, p.x, p.y, p.r, heat);
      else {
        ctx.beginPath();
  ...
  function drawGauge(cx, cy, P) {
    var art = n1();
    if (art) {
      art.drawBourdon(ctx, cx, cy, P, lang === "zh" ? "Bourdon" : "Bourdon");
      return;
    }
    ctx.beginPath();
    ctx.fillStyle = "#fff";
  ...
  function drawGraph() {
    ...
    gctx.font = art ? art.font(800, 12) : "800 12px Plus Jakarta Sans, sans-serif";
    gctx.textAlign = "left";
    ...
    gctx.font = art ? art.mono(600, 11) : "600 11px JetBrains Mono, monospace";
    ...
    for (var j = 0; j < trail.length; j++) {
      ...
      gctx.beginPath();
      gctx.fillStyle = axes.dot;
      gctx.globalAlpha = 0.28 + (j / Math.max(1, trail.length)) * 0.55;
    ...
    gctx.fillStyle = "#0f172a";
    gctx.font = art ? art.font(800, 11) : "800 11px Plus Jakarta Sans, sans-serif";
