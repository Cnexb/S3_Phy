/** @type {Record<string, string>} */
export const zhHant = {
  'app.title': 'Uni+ HKDSE \u7269\u7406\u5b78\u7fd2\u4e2d\u5fc3',
  'app.subtitle': '\u4e2d\u4e09\u7269\u7406 \u2014 \u57fa\u790e\u3001\u5149\u8207\u5149\u5b78\u3001\u71b1',
  'strands.title': '\u9078\u64c7\u55ae\u5143',
  'strands.intro': '\u7269\u7406\u57fa\u790e\u3001\u5149\u8207\u5149\u5b78\u3001\u71b1\u5b78\u5df2\u53ef\u4f7f\u7528\u3002\u5176\u4ed6\u55ae\u5143\u5373\u5c07\u63a8\u51fa\uff0c\u4ecd\u53ef\u9032\u5165\u9810\u89bd\u3002',
  'strand.optics.title': '\u5149\u8207\u5149\u5b78',
  'strand.optics.desc': '\u53cd\u5c04\u3001\u6298\u5c04\u3001\u5168\u5167\u53cd\u5c04\u3001\u51f8\u900f\u93e1\u3001\u51f9\u900f\u93e1\u3001\u96fb\u78c1\u6ce2',
  'strand.optics.subtitle': '\u5149\u8207\u5149\u5b78 \u2014 \u5149\u8207\u6ce2\u52d5',
  'strand.refraction.title': '\u6298\u5c04',
  'strand.refraction.desc': '\u5149\u7684\u6298\u5c04\u3001\u53f8\u4e43\u8033\u5b9a\u5f8b\u3001\u81e8\u754c\u89d2\u8207\u5168\u5167\u53cd\u5c04\u3002',
  'strand.refraction.subtitle': '\u6298\u5c04 \u2014 \u53f8\u4e43\u8033\u5b9a\u5f8b\u8207\u5168\u5167\u53cd\u5c04',
  'strand.heat.title': '\u71b1\u5b78',
  'strand.heat.desc': '\u6eab\u5ea6\u8a08\u3001\u71b1\u8207\u5167\u80fd\u3001\u7269\u614b\u8b8a\u5316\u3001\u71b1\u50b3\u5165',
  'strand.heat.subtitle': '\u71b1\u5b78 \u2014 \u6eab\u5ea6\u8207\u71b1\u50b3\u5165',
  'strand.open': '\u9032\u5165\u55ae\u5143',
  'strand.back': '\u5168\u90e8\u55ae\u5143',
  'strand.comingSoon': '\u5373\u5c07\u63a8\u51fa',
  'strand.notReady': '\u5c1a\u672a\u958b\u653e',
  'heat.section.empty': '\u71b1\u5b78\u55ae\u5143\u5167\u5bb9\u3002',
  'splash.start': '\u958b\u59cb',
  'splash.unitEducation': 'Unit Education',
  'lang.en': 'English',
  'lang.zhHant': '\u7e41\u9ad4\u4e2d\u6587',
  'profile.open': '\u5e33\u6236\u9078\u55ae',
  'profile.student': '\u5b78\u751f',
  'profile.guest': '\u8a2a\u5ba2',
  'profile.settings': '\u8a2d\u5b9a',
  'profile.language': '\u8a9e\u8a00',
  'profile.appearance': '\u5916\u89c0',
  'profile.themeToDark': '\u5207\u63db\u81f3\u6df1\u8272\u6a21\u5f0f',
  'profile.themeToLight': '\u5207\u63db\u81f3\u6dfa\u8272\u6a21\u5f0f',
  'profile.logout': '\u767b\u51fa',
  'nav.topics': '\u8ab2\u984c',
  'nav.notes': '\u7b46\u8a18',
  'nav.tools': '\u4e92\u52d5\u5be6\u9a57\u5ba4',
  'nav.worksheets': '\u5de5\u4f5c\u7d19',
  'nav.quiz': '\u6e2c\u9a57',
  'nav.flashcards': '\u9583\u5361',
  'nav.summary': '\u7e3d\u7d50',
  'nav.comics': '\u6f2b\u756b',
  'topics.title': '\u8ab2\u984c\u5730\u5716',
  'topics.intro': '\u9078\u64c7\u5b78\u7fd2\u4e3b\u984c\u3002\u6bcf\u500b\u4e3b\u984c\u53ef\u9023\u5230\u4e92\u52d5\u5be6\u9a57\u5ba4\u3002',
  'topic.reflection': '\u53cd\u5c04',
  'topic.reflection3d': '3D \u53cd\u5c04\u5be6\u9a57\u5ba4',
  'topic.rotatingMirror': '\u65cb\u8f49\u5e73\u9762\u93e1',
  'topic.refraction': '\u6298\u5c04',
  'topic.refractionSnell': '\u6298\u5c04\u8207\u53f8\u4e43\u8033\u5b9a\u5f8b',
  'topic.tir': '\u5168\u53cd\u5c04',
  'topic.mirage': '\u6d77\u5e02\u8718\u6a13\uff08\u71b1\uff0f\u51b7\u7a7a\u6c23\u5168\u5167\u53cd\u5c04\uff09',
  'topic.refractionTir': '\u6298\u5c04\u3001\u53f8\u4e43\u8033\u5b9a\u5f8b\u8207\u5168\u5167\u53cd\u5c04',
  'topic.convex': '\u51f8\u900f\u93e1',
  'topic.concave': '\u51f9\u900f\u93e1',
  'topic.em': '\u96fb\u78c1\u6ce2',
  'topic.openTool': '\u958b\u555f\u5be6\u9a57\u5ba4',
  'notes.title': '\u5b78\u7fd2\u7b46\u8a18\uff08PDF\uff09',
  'notes.card.reflection': 'JPWM06 \u5149\u7684\u53cd\u5c04',
  'notes.card.refraction': '\u6298\u5c04\u8207\u53f8\u4e43\u8033\u5b9a\u5f8b',
  'notes.card.tir': '\u5168\u5167\u53cd\u5c04',
  'notes.card.convexLens': '\u51f8\u900f\u93e1',
  'notes.card.concaveLens': '\u51f9\u900f\u93e1',
  'notes.card.em': '\u96fb\u78c1\u6ce2\u8b5c',
  'notes.openPdf': '\u958b\u555f PDF',
  'notes.downloadPdf': '\u4e0b\u8f09 PDF',
  'notes.embedHint': '\u82e5\u9810\u89bd\u7a7a\u767d\uff0c\u8acb\u958b\u555f\u6216\u4e0b\u8f09 PDF\u3002',
  'notes.missing': '\u5c1a\u672a\u4e0a\u8f09 PDF\u3002',
  'notes.comingSoon': '\u6b64\u55ae\u5143\u7684\u5b78\u7fd2\u7b46\u8a18\u5373\u5c07\u63a8\u51fa\u3002',
  'tools.title': '\u4e92\u52d5\u5be6\u9a57\u5ba4',
  'tools.pick': '\u9078\u64c7\u5be6\u9a57',
  'tools.hideLabList': '\u6536\u5408\u5be6\u9a57\u5217\u8868',
  'tools.showLabList': '\u986f\u793a\u5be6\u9a57\u5217\u8868',
  'tools.fullscreen': '\u5168\u87fb\u5e55',
  'tools.exitFullscreen': '\u9000\u51fa\u5168\u87fb\u5e55',
  'tools.rotatingMirror.title': '\u65cb\u8f49\u5e73\u9762\u93e1',
  'tools.rotatingMirror.subtitle': '\u93e1\u9762\u8f49\u52d5\u6642\u53cd\u5c04\u5149\u7dda\u7684\u504f\u79fb\uff08\u5149\u7dda\u504f\u89d2 = 2 \u00d7 \u93e1\u89d2\uff09\u3002',
  'tools.planeMirror.title': '\u5e73\u9762\u93e1\u53cd\u5c04\u5be6\u9a57\u5ba4',
  'tools.planeMirror.subtitle':
    '\u50cf\u7684\u5f62\u6210\u3001\u6700\u5c0f\u93e1\u9577\uff0f\u93e1\u9ad8\u3001\u770b\u5f8c\u65b9\u7269\u4ef6\u3001\u593e\u89d2\u93e1\u8207\u7df4\u7fd2\u4f5c\u5716\uff0c\u914d\u5408\u5373\u6642\u516c\u5f0f\u3002',
  'tools.reflection3d.title': '3D \u53cd\u5c04\u5be6\u9a57\u5ba4',
  'tools.reflection3d.subtitle': '3D \u6a21\u64ec\u6f2b\u53cd\u5c04\u8207\u93e1\u9762\u53cd\u5c04\uff0c\u4ee5\u53ca\u586b\u5145\u4ecb\u8cea\u7684\u5e73\u6ed1\u5316\u904e\u7a0b\u3002',
  'tools.electrostatics.title': '靜電',
  'tools.domesticCircuit.title': '家居電路',
  'tools.magneticEffect.title': '電流的磁效應',
  'tools.movingChargeForce.title': '磁場對運動電荷的作用力',
  'tools.flemingHandRules.title': '\u5f17\u6797\u660e\u5de6\u53f3\u624b\u6cd5\u5247',
  'tools.flemingHandRules.subtitle':
    '3D \u5de6\u624b\u5b9a\u5247\uff08\u96fb\u52d5\u6a5f\uff09\u3001\u53f3\u624b\u5b9a\u5247\uff08\u767c\u96fb\u6a5f\uff09\u53ca\u53f3\u624b\u63e1\u62f3\u5b9a\u5247\uff08HKDSE\uff09\u3002',
  'tools.radiationDeflection.title': '\u8f3b\u5c04\u504f\u8f49\uff08\u03b1 \u03b2 \u03b3\uff09',
  'tools.radiationDeflection.subtitle':
    '\u03b1\u3001\u03b2\u3001\u03b3 \u5728\u96fb\u5834\u8207\u78c1\u5834\u4e2d\u7684\u504f\u8f49\u3002',
  'tools.cloudChamber.title': '\u96f2\u5ba4\uff08\u03b1 \u03b2 \u03b3\uff09',
  'tools.cloudChamber.subtitle':
    '\u03b1\u3001\u03b2\u3001\u03b3 \u5728\u96f2\u5ba4\u4e2d\u7684\u5f91\u8de1\u3002',
  'tools.rotatingMirror.toggle.origNormal': '\u539f\u6cd5\u7dda',
  'tools.rotatingMirror.toggle.origRay': '\u539f\u53cd\u5c04\u5149',
  'tools.rotatingMirror.toggle.newNormal': '\u65b0\u6cd5\u7dda',
  'tools.rotatingMirror.toggle.newRay': '\u65b0\u53cd\u5c04\u5149',
  'tools.rotatingMirror.incidentAngle': '\u5165\u5c04\u65b9\u5411\uff08\u00b0\uff09',
  'tools.rotatingMirror.mirrorRotation': '\u93e1\u9762\u8f49\u89d2 \u03b8\uff08\u00b0\uff09',
  'tools.rotatingMirror.statIncidence': '\u5165\u5c04\u89d4 i',
  'tools.rotatingMirror.statReflection': '\u53cd\u5c04\u89d4 r',
  'tools.rotatingMirror.rayShift': '\u5149\u7dda\u504f\u79fb',
  'tools.rotatingMirror.logicBadge': '\u95dc\u4fc2\uff1a2 \u00d7 \u03b8',
  'tools.rotatingMirror.reset': '\u91cd\u8a2d\u8f49\u89d2',
  'tools.rotatingMirror.canvas.incident': '\u5165\u5c04\u5149\u7dda',
  'tools.rotatingMirror.canvas.originalReflected': '\u539f\u53cd\u5c04\u5149\u7dda',
  'tools.rotatingMirror.canvas.newReflected': '\u65b0\u53cd\u5c04\u5149\u7dda',
  'tools.rotatingMirror.canvas.n': 'n',
  'tools.rotatingMirror.canvas.nPrime': 'n\u2032',
  'tools.rotatingMirror.canvas.shift': '\u504f\u79fb',
  'tools.refractionTir.title': '\u9003\u7344\u904a\u6232 \u2014 \u6298\u5c04\u3001\u53f8\u4e43\u8033\u8207\u5168\u5167\u53cd\u5c04',
  'tools.mirage.title': '\u6d77\u5e02\u8718\u6a13 \u2014 \u71b1\uff0f\u51b7\u7a7a\u6c23\u5168\u5167\u53cd\u5c04',
  'tools.refraction.title': '\u6298\u5c04 \u2014 \u53f8\u4e43\u8033\u5b9a\u5f8b',
  'tools.waveMotion.title': '\u6ce2\u52d5\u904b\u52d5 \u2014 \u76f8\u4f4d\u5206\u6790',
  'tools.rippleTank.title': '水波槽 — 波動特性',
  'tools.longitudinalWave.title': '\u7e26\u6ce2 \u2014 \u58d3\u7e2e\u8207\u7a00\u758e',
  'tools.ultrasoundReflection.title': '\u8d85\u8072\u6ce2\u53cd\u5c04',
  'tools.waveInterference.title': '\u7a7a\u9593\u6ce2\u7684\u5e72\u6d09',
  'tools.waveDiffraction.title': '\u6ce2\u7684\u885d\u5c04 \u2014 \u7e2b\u5bec\u8207\u6ce2\u9577',
  'tools.stationaryWave.title': '\u99d0\u6ce2 \u2014 \u5f26\u4e0a\u7684\u99d0\u6ce2',
  'tools.thinFilmInterference.title': '\u8584\u819c\u5e72\u6d89 \u2014 \u80a5\u7682\u6ce1',
  'tools.diffractionGratingCompare.title': '\u96d9\u72f9\u7e2b\u8207\u5149\u6805\u6bd4\u8f03',
  'tools.spectralOverlap.title': '\u5149\u8b5c\u91cd\u758a \u2014 \u7e5e\u5c04\u7d1a\u6b21',
  'tools.soundRefractionShadow.title': '\u8072\u6ce2\u6298\u5c04 \u2014 \u9670\u5f71\u5340',
  'tools.refraction.n1': '\u5165\u5c04\u4ecb\u8cea',
  'tools.refraction.n2': '\u6298\u5c04\u4ecb\u8cea',
  'tools.refraction.angle': '\u5165\u5c04\u89d4\uff08\u5ea6\uff09',
  'tools.refraction.angleI': '\u5165\u5c04\u89d4 \u03b8\u2081\uff08\u00b0\uff09',
  'tools.refraction.angleR': '\u6298\u5c04\u89d4 \u03b8\u2082\uff08\u00b0\uff09',
  'tools.refraction.medium.air': '\u7a7a\u6c23',
  'tools.refraction.medium.water': '\u6c34',
  'tools.refraction.medium.glass': '\u73bb\u7483',
  'tools.refraction.speedLabel': '\u5149\u901f v',
  'tools.refraction.tirNote': '\u5168\u5167\u53cd\u5c04 \u2014 \u6c92\u6709\u6298\u5c04\u5149\u7dda\uff08\u03b8\u2081 > \u03b8c\uff09\u3002',
  'tools.tir.title': '\u5168\u5167\u53cd\u5c04',
  'tools.tir.n1': 'n \u5bc6\uff08\u5149\u5bc6\uff09',
  'tools.tir.n2': 'n \u758f\uff08\u5149\u758f\uff09',
  'tools.tir.angle': '\u5149\u5bc6\u4ecb\u8cea\u5167\u5165\u5c04\u89d4\uff08\u5ea6\uff09',
  'tools.tir.critical': '\u81e8\u754c\u89d4',
  'tools.tir.state': '\u72c0\u614b',
  'tools.tir.partial': '\u90e8\u5206\u6298\u5c04',
  'tools.tir.full': '\u5168\u5167\u53cd\u5c04',
  'tools.lens.title': '\u900f\u93e1 \u2014 \u5149\u7dda\u793a\u610f',
  'tools.convexLens.title': '\u51f8\u900f\u93e1 \u2014 \u5149\u7dda\u793a\u610f',
  'tools.concaveLens.title': '\u51f9\u900f\u93e1 \u2014 \u5149\u7dda\u793a\u610f',
  'tools.lens.type': '\u900f\u93e1\u985e\u578b',
  'tools.lens.convex': '\u51f8\u900f\u93e1\uff08\u6703\u805a\uff09',
  'tools.lens.concave': '\u51f9\u900f\u93e1\uff08\u767c\u6563\uff09',
  'tools.lens.u': '\u7269\u8ddd u\uff08\u5398\u7c73\uff09',
  'tools.lens.f': '\u7126\u8ddd\u5927\u5c0f f\uff08\u5398\u7c73\uff09',
  'tools.lens.h': '\u7269\u9ad8\uff08\u76f8\u5c0d\uff09',
  'tools.lens.note': '\u7b26\u865f\uff1a\u5be6\u7269\u3001\u5be6\u50cf\u8ddd\u96e2\u53d6\u6b63\uff1b\u51f8\u900f\u93e1 f \u70ba\u6b63\uff0c\u51f9\u900f\u93e1 f \u70ba\u8ca0\u3002\u5716\u793a\u70ba\u5b9a\u6027\u793a\u610f\u3002',
  'tools.rgbMixer.title': 'RGB \u984f\u8272\u6df7\u5408',
  'tools.rgbMixer.intro': '\u52a0\u6cd5\u6df7\u8272\uff1a\u7d05\u3001\u7da0\u3001\u85cd\u5149\u5728\u87fb\u5e55\u4e0a\u758a\u52a0\u3002\u62d6\u52d5\u6ed1\u68d2\u6216\u4f7f\u7528\u9810\u8a2d\u3002',
  'tools.rgbMixer.canvas': 'RGB \u5149\u7dda\u758a\u52a0\u793a\u610f\u5716',
  'tools.rgbMixer.controls': '\u984f\u8272\u63a7\u5236',
  'tools.rgbMixer.red': '\u7d05',
  'tools.rgbMixer.green': '\u7da0',
  'tools.rgbMixer.blue': '\u85cd',
  'tools.rgbMixer.preview': '\u6df7\u5408\u984f\u8272\u9810\u89bd',
  'tools.rgbMixer.presets': '\u9810\u8a2d',
  'tools.rgbMixer.presetRed': '\u7d05',
  'tools.rgbMixer.presetGreen': '\u7da0',
  'tools.rgbMixer.presetBlue': '\u85cd',
  'tools.rgbMixer.presetYellow': '\u9ec3',
  'tools.rgbMixer.presetCyan': '\u9752',
  'tools.rgbMixer.presetMagenta': '\u54c1\u7d05',
  'tools.rgbMixer.presetWhite': '\u767d',
  'tools.rgbMixer.presetBlack': '\u9ed1',
  'topic.rgbMixer': 'RGB \u984f\u8272\u6df7\u5408',
  'tools.em.title': '\u96fb\u78c1\u6ce2\u8b5c',
  'tools.em.pick': '\u63a2\u7d22\u6ce2\u9577\u3001\u983b\u7387\u8207\u6a21\u64ec\u5be6\u9a57',
  'tools.rayDiagram.title': '\u5e73\u9762\u93e1\u6210\u50cf',
  'tools.rayDiagram.pick': '\u5728\u65b9\u683c\u4e0a\u756b\u5e73\u9762\u93e1\u5149\u7dda\u5716',
  'worksheets.practiceTitle': '\u4e92\u52d5\u7df4\u7fd2',
  'worksheets.foundationsNotesTitle': '\u4e2d\u4e09\u7269\u7406\u57fa\u790e \u00b7 \u7269\u7406\u91cf\u3001\u55ae\u4f4d\u8207\u5e38\u7528\u6578\u5b78',
  'worksheets.pick': '\u9078\u64c7\u5de5\u4f5c\u7d19',
  'worksheets.opticsLightLensTitle': '\u4e2d\u4e09\u5149\u5b78 \u7b2c\u4e09\u7ae0 \u5149\u8207\u900f\u93e1',
  'worksheets.opticsEmWaveTitle': '\u4e2d\u4e09\u5149\u5b78 \u7b2c\u56db\u7ae0 \u96fb\u78c1\u6ce2',
  'quiz.practiceTitle': 'JPHG01 溫度與溫度計',
  'quiz.practiceTitle2': 'JPHG01 溫度與溫度計 · 測驗 2',
  'quiz.practiceTitle3': 'JPHG01 溫度與溫度計 · 測驗 3',
  'quiz.jphg01Quiz1': 'JPHG01 測驗 1',
  'quiz.jphg01Quiz2': 'JPHG01 測驗 2',
  'quiz.jphg01Quiz3': 'JPHG01 測驗 3',
  'quiz.opticsCh3Title': 'JPWM06 光的反射',
  'quiz.opticsCh3Title2': 'JPWM06 光的反射 · 測驗 2',
  'quiz.jpwm06Quiz1': 'JPWM06 測驗 1',
  'quiz.jpwm06Quiz2': 'JPWM06 測驗 2',
  'quiz.foundationsTitle': '\u4e2d\u4e09\u7269\u7406\u57fa\u790e\u6e2c\u9a57',
  'quiz.foundationsUsefulMathTitle': 'F02 \u7269\u7406\u4e2d\u5e38\u7528\u7684\u6578\u5b78',
  'quiz.label.quantitiesUnits': 'F01 \u7269\u7406\u91cf\u8207\u55ae\u4f4d',
  'quiz.label.usefulMaths': 'F02 \u7269\u7406\u4e2d\u5e38\u7528\u7684\u6578\u5b78',
  'quiz.pick': '\u9078\u64c7\u6e2c\u9a57',
  'quiz.comingSoon': '\u6b64\u55ae\u5143\u7684\u4e92\u52d5\u6e2c\u9a57\u5c07\u65bc\u65e5\u5f8c\u63a8\u51fa\u3002',
  'flashcards.title': '\u9583\u5361',
  'flashcards.intro': '\u4f7f\u7528\u9593\u9694\u91cd\u8907\u7fd2\u2014\u96e3\u8a18\u7684\u5361\u7247\u5728\u4e0b\u4e00\u8f2a\u6703\u66f4\u983b\u7e41\u51fa\u73fe\u3002\u5305\u542b\u5149\u7b2c3\u7ae0\u8ab2\u984c\u53ca\u51f8\u900f\u93e1\u3001\u51f9\u900f\u93e1\u3001\u96fb\u78c1\u6ce2\u5b9a\u7fa9\u5361\u3002',
  'flashcards.progress': '\u7b2c {current} \u5f35\uff0c\u5171 {total} \u5f35',
  'flashcards.deck': '\u5361\u7d44',
  'flashcards.question': '\u554f\u984c',
  'flashcards.answer': '\u7b54\u6848',
  'flashcards.flip': '\u7ffb\u9762',
  'flashcards.next': '\u4e0b\u4e00\u5f35',
  'flashcards.prev': '\u4e0a\u4e00\u5f35',
  'flashcards.all': '\u5168\u90e8\u8ab2\u984c',
  'flashcards.deck.refractionTir': '\u6298\u5c04\u8207\u5168\u5167\u53cd\u5c04',
  'flashcards.deck.definitions': '\u900f\u93e1\u8207\u96fb\u78c1\u6ce2\uff08\u5b9a\u7fa9\uff09',
  'flashcards.mode.sequence': '\u9806\u5e8f',
  'flashcards.mode.random': '\u96a8\u6a5f',
  'flashcards.again': '\u518d\u4f86',
  'flashcards.gotIt': '\u61c2\u4e86',
  'flashcards.round': '\u7b2c {round} \u8f2a \u00b7 {total} \u5f35\u5361',
  'flashcards.studyMode': '\u5b78\u7fd2\u6a21\u5f0f',
  'flashcards.progressLabel': '\u9032\u5ea6',
  'flashcards.tapFlip': '\u9ede\u64ca\u3001\u7a7a\u767d\u9375\u6216 FLIP \u986f\u793a\u7b54\u6848',
  'flashcards.cardCode': '\u5361\u7247 {id}',
  'flashcards.flipFirst': '\u8acb\u5148\u9ede\u64ca\u5361\u7247\u3001\u6309\u7a7a\u767d\u9375\u6216\u7ffb\u9762\u67e5\u770b\u7b54\u6848\u3002',
  'flashcards.hint.rated': '\u2190 \u518d\u4f86 \u00b7 \u61c2\u4e86 \u2192\uff08\u6216\u6309 1 / 2\uff09',
  'flashcards.summary.title': '\u7b2c {round} \u8f2a\u5b8c\u6210',
  'flashcards.summary.stats': '\u5171 {total} \u5f35 \u00b7 \u518d\u4f86 {again} \u00b7 \u61c2\u4e86 {gotIt} \u00b7 \u4e0b\u4e00\u5f35 {neutral}',
  'flashcards.summary.keep': '{count} \u5f35\u9700\u7e7c\u7e8c\u7df4\u7fd2',
  'flashcards.summary.confident': '{count} \u5f35\u5df2\u638c\u63e1',
  'flashcards.summary.nextRound': '\u4e0b\u4e00\u8f2a',
  'flashcards.summary.restart': '\u91cd\u65b0\u958b\u59cb',
  'flashcards.summary.complete': '\u7df4\u7fd2\u5b8c\u6210',
  'flashcards.summary.completeStats': '\u5f88\u597d\uff01\u4f60\u5df2\u5b8c\u6210\u6240\u6709\u6392\u7a0b\u7684\u5361\u7247\u3002',
  'summary.title': '\u8ab2\u984c\u7e3d\u7d50',
  'summary.intro': 'HKDSE \u7e3d\u7d50\u8868\uff08PDF \u6216\u5716\u7247\uff09\u3002\u53ef\u65bc\u4e0b\u65b9\u9810\u89bd\uff0c\u6216\u65b0\u5206\u9801\u958b\u555f\u3002',
  'summary.viewImage': '\u958b\u555f\u5b8c\u6574\u5716\u7247',
  'summary.download': '\u958b\u555f / \u4e0b\u8f09 PDF',
  'summary.missing': '\u627e\u4e0d\u5230\u7e3d\u7d50 PDF\u3002',
  'comics.title': '\u6f2b\u756b',
  'comics.intro': '\u6b64\u55ae\u5143\u7684\u8ab2\u984c\u6f2b\u756b\u3002\u53ef\u65bc\u4e0b\u65b9\u9810\u89bd\uff0c\u6216\u65b0\u5206\u9801\u958b\u555f\u3002',
  'comics.viewImage': '\u958b\u555f\u5b8c\u6574\u5716\u7247',
  'comics.openPdf': '\u958b\u555f / \u4e0b\u8f09 PDF',
  'comics.missing': '\u5c1a\u672a\u4e0a\u50b3\u6f2b\u756b\u3002',
  'footer.conventions': '\u900f\u93e1\u5149\u7dda\u5716\u63a1\u7528\u8207\u5404\u5be6\u9a57\u5ba4\u8aaa\u660e\u4e00\u81f4\u7684\u8584\u900f\u93e1\u7b26\u865f\u898f\u5b9a\u3002',
  'summary.item.reflection': '\u53cd\u5c04',
  'summary.item.refraction': '\u6298\u5c04\u8207\u53f8\u4e43\u8033\u5b9a\u5f8b',
  'summary.item.tir': '\u5168\u5167\u53cd\u5c04',
  'summary.item.convex': '\u51f8\u900f\u93e1',
  'summary.item.concave': '\u51f9\u900f\u93e1',
  'summary.item.em': '\u96fb\u78c1\u6ce2',
  'summary.item.initialD': '\u982d\u6587\u5b57 D\uff1a\u6392\u6c34\u6e9d\u904e\u5f4e \u2014 \u53cd\u5c04\u8207\u6298\u5c04',
  // Heat translations
  'topic.thermometer': '\u6eab\u5ea6\u8a08',
  'topic.heatInternalEnergy': '\u71b1\u8207\u5167\u80fd',
  'topic.changeOfState': '\u7269\u614b\u8b8a\u5316',
  'topic.heatTransfer': '\u71b1\u50b3\u5165',
  'topic.thermometerLab': '\u6eab\u5ea6\u8a08\u8207\u6821\u6e96\u5be6\u9a57\u5ba4',
  'topic.viewNotes': '\u67e5\u95b1\u7b46\u8a18',
  'notes.card.thermometer': 'JPHG01 \u6eab\u5ea6\u8207\u6eab\u5ea6\u8a08',
  'notes.card.heatInternalEnergy': 'Ch.1.2 \u71b1\u8207\u5167\u80fd',
  'notes.card.changeOfState': 'Ch.1.3 \u7269\u614b\u8b8a\u5316',
  'notes.card.heatTransfer': 'Ch.1.4 \u71b1\u50b3\u5165',
  'summary.item.thermometer': '\u6eab\u5ea6\u8a08',
  'summary.item.heatInternalEnergy': '\u71b1\u8207\u5167\u80fd',
  'summary.item.changeOfState': '\u7269\u614b\u8b8a\u5316',
  'summary.item.heatTransfer': '\u71b1\u50b3\u5165',
  'flashcards.deck.thermometry': '\u6eab\u5ea6\u8a08\uff08\u5404\u985e\uff09',
  'flashcards.deck.heatInternalEnergy': '\u71b1\u8207\u5167\u80fd',
  'flashcards.deck.changeOfState': '\u7269\u614b\u8b8a\u5316',
  'flashcards.deck.heatTransfer': '\u71b1\u50b3\u5165',
  'tools.thermometerLab.title': '\u6eab\u5ea6\u8a08\u8207\u6821\u6e96\u5be6\u9a57\u5ba4',
  'tools.boilingWater.title': '煲水能量實驗室',
  'tools.heatingMaterials.title': '比熱容量與溫度差之間的關係',
  'tools.specificHeat.title': '比熱容量實驗室',
  'tools.thermalMixing.title': '熱混合實驗室',
  'tools.changeOfState.title': '物態變化實驗室',
  'tools.heatFlow.title': '熱能流動與熱平衡',
  'tools.heatTransfer.title': '熱的轉移實驗室',
  'tools.thermometerLab.subtitle': '\u6db2\u9ad4\u73bb\u7483\u3001\u9210\u96fb\u963b\u53ca\u71b1\u654f\u96fb\u963b\u6eab\u5ea6\u8a08\u7684\u4e92\u52d5\u5b78\u7fd2\u4f34\u4fb6\u3002',
  'tools.thermometerLab.liquid.title': '\u6eab\u5ea6\u8a08\u8a2d\u8a08',
  'tools.thermometerLab.liquid.subtitle': '\u8abf\u8f03\u73bb\u7483\u6ce1\u3001\u6bdb\u7d30\u7ba1\u540c\u73bb\u7483\u58c1\uff0c\u7747\u9748\u654f\u5ea6\u3001\u91cf\u7a0b\u540c\u53cd\u61c9\u9ede\u6a23\u8b8a\uff08HKDSE\uff09\u3002',
  'tools.thermometerLab.resistance.title': '\u9210\u96fb\u963b\u6eab\u5ea6\u8a08',
  'tools.thermometerLab.resistance.subtitle': '\u7814\u7a76\u9210\u96fb\u963b\u5982\u4f55\u96a8\u6eab\u5ea6\u5448\u7dda\u6027\u8b8a\u5316\uff0c\u4e26\u89e3\u6c7a\u6eab\u5ea6\u6821\u6e96\u554f\u984c\u3002',
  'tools.thermometerLab.thermistor.title': '\u8ca0\u6eab\u5ea6\u4fc2\u6578\u71b1\u654f\u96fb\u963b\u6eab\u5ea6\u8a08',
  'tools.thermometerLab.thermistor.subtitle': '\u5b78\u7fd2\u8ca0\u6eab\u5ea6\u4fc2\u6578\u71b1\u654f\u96fb\u963b\u7684\u975e\u7dda\u6027\u96fb\u963b-\u6eab\u5ea6\u95dc\u4fc2\uff0c\u4e26\u9032\u884c\u5be6\u6642 Beta \u8a08\u7b97\u3002',
  'tools.thermometerLab.thermistor.liveBetaLabel': '\u5373\u6642\u8ca0\u6eab\u5ea6\u4fc2\u6578 Beta \u8a08\u7b97',
  'tools.thermometerLab.thermistor.meterLabelLine1': '\u8ca0\u6eab\u5ea6\u4fc2\u6578',
  'tools.thermometerLab.thermistor.meterLabelLine2': '\u71b1\u654f\u96fb\u963b',
  'tools.thermometerLab.labels.show': '\u986f\u793a\u7d50\u69cb\u6a19\u8a3b',
  'tools.thermometerLab.labels.hide': '\u96b1\u85cf\u7d50\u69cb\u6a19\u8a3b',
  'tools.thermometerLab.labels.thinWall': '\u8584\u73bb\u7483\u58c1\uff08\u50b3\u71b1\u66f4\u5feb \u2192 \u53cd\u61c9\u66f4\u5feb\uff09',
  'tools.thermometerLab.labels.largeBulb': '\u5927\u73bb\u7483\u6ce1\uff08\u6db2\u9ad4\u8f03\u591a \u2192 \u9748\u654f\u5ea6\u2191\uff09',
  'tools.thermometerLab.labels.narrowBore': '\u7d30\u6bdb\u7d30\u7ba1\uff08\u6db2\u67f1\u5347\u5f97\u66f4\u9ad8 \u2192 \u9748\u654f\u5ea6\u2191\uff09',
  'tools.thermometerLab.labels.meniscus': '\u5f4e\u6708\u9762 (\u8b80\u6578\u4f4d\u7f6e)',
  'tools.thermometerLab.labels.platinumCoil': '\u9210\u7d72\u7dda\u5708 (\u96fb\u963b\u96a8\u6eab\u5ea6\u7dda\u6027\u589e\u52a0)',
  'tools.thermometerLab.labels.metalSheath': '\u91d1\u5c6c\u4fdd\u8b77\u5957 (\u4fdd\u8b77\u4e26\u5feb\u901f\u5c0e\u71b1)',
  'tools.thermometerLab.labels.semiconductorBead': '\u534a\u5c0e\u9ad4\u5c0f\u73e0 (\u96fb\u963b\u96a8\u6eab\u5ea6\u6307\u6578\u7d1a\u6e1b\u5c11)',
  'tools.thermometerLab.labels.leads': '\u9023\u63a5\u5c0e\u7dda',
  'tools.thermometerLab.faulty.dualScale': '\u96d9\u523b\u5ea6\u5c0d\u6bd4\u793a\u610f\u5716',
  'tools.thermometerLab.faulty.trueScale': '\u6a19\u6e96\u6eab\u6a19 (\u00b0C)',
  'tools.thermometerLab.faulty.faultyScale': '\u504f\u5dee\u6eab\u6a19 (\u00b0C)',
  'tools.thermometerLab.paramSettings': '\u53c3\u6578\u8a2d\u5b9a',
  'tools.floatingControls.dragHint': '\u62d6\u66f3\u4ee5\u79fb\u52d5',
  'tools.floatingControls.showParams': '\u986f\u793a\u53c3\u6578',
  'tools.floatingControls.hideParams': '\u96b1\u85cf\u53c3\u6578',
  'tools.thermometerLab.faultySolver': '\u6545\u969c\u6eab\u5ea6\u8a08\u6c42\u89e3\u5668',
  'tools.thermometerLab.resistanceSolver': '\u96fb\u963b\u6c42\u6eab\u5ea6\u6c42\u89e3\u5668',
  'tools.faultyCalibration.title': '\u504f\u5dee\u6eab\u6a19\u6821\u6e96',
  'tools.faultyCalibration.subtitle': '\u8a2d\u5b9a\u51b0\u9ede\u8207\u6c34\u84b8\u9ede\u7684\u504f\u5dee\u8b80\u6578\uff0c\u6c42\u89e3\u771f\u5be6\u6eab\u5ea6 T \u6216\u504f\u5dee\u8b80\u6578 C\u3002',
  'tools.faultyCalibration.formulaTitle': '\u504f\u5dee\u6eab\u6a19\u6821\u6e96',
  'tools.faultyCalibration.intro': '\u8a2d\u5b9a\u51b0\u9ede\uff080\u00b0C\uff09\u8207\u6c34\u84b8\u9ede\uff08100\u00b0C\uff09\u7684\u504f\u5dee\u8b80\u6578\uff0c\u7136\u5f8c\u6c42\u89e3 T \u6216 C\u3002',
  'tools.faultyCalibration.liveLabel': '\u5373\u6642\u8a08\u7b97\u904e\u7a0b',
  'tools.faultyCalibration.iceReading': '\u51b0\u9ede\u8b80\u6578 C<sub>f</sub>\uff08\u771f\u5be6 0\u00b0C\uff09',
  'tools.faultyCalibration.steamReading': '\u6c34\u84b8\u9ede\u8b80\u6578 C<sub>u</sub>\uff08\u771f\u5be6 100\u00b0C\uff09',
  'tools.faultyCalibration.interval': '\u6bd4\u4f8b\u5340\u9593\uff08C<sub>u</sub> \u2212 C<sub>f</sub>\uff09',
  'tools.faultyCalibration.findTrueTemp': '\u6c42\u771f\u5be6\u6eab\u5ea6\uff08T\uff09',
  'tools.faultyCalibration.findFaultyReading': '\u6c42\u504f\u5dee\u8b80\u6578\uff08C\uff09',
  'tools.faultyCalibration.faultyReadingC': '\u504f\u5dee\u8b80\u6578 C',
  'tools.faultyCalibration.trueTempT': '\u771f\u5be6\u6eab\u5ea6 T',
  'tools.faultyCalibration.invalidInterval': 'C<sub>u</sub> \u8207 C<sub>f</sub> \u4e0d\u53ef\u76f8\u540c\uff08\u5340\u9593\u4e0d\u53ef\u70ba\u96f6\uff09\u3002',
  // Foundations translations
  'strand.foundations.title': '物理基礎',
  'strand.foundations.desc': '物理量與單位、物理中常用的數學',
  'strand.foundations.subtitle': '物理基礎 — 物理量、單位與常用數學',
  'foundations.section.empty': '物理基礎單元內容即將推出。',
  'foundations.comingSoon': '此單元的互動實驗室將於日後推出。',
  'tools.siUnits.title': '物理量與單位',
  'flashcards.introFoundations': '使用間隔重複學習——難記的卡片在下一輪會更頻繁出現。涵蓋 Ch.00 物理量與單位，以及物理常用數學。',
  'flashcards.introMechanics': '使用間隔重複習——難記的卡片在下一輪會更頻繁出現。涵蓋力學全部 13 章，由位置與運動至萬有引力。',
  'topic.quantitiesUnits': '物理量與單位',
  'topic.usefulMaths': '物理中常用的數學',
  'notes.card.quantitiesUnits': 'F01 筆記-物理量與單位',
  'notes.card.usefulMaths': 'F02 筆記-物理中常用的數學',
  'summary.item.quantitiesUnits': '物理量與單位',
  'summary.item.usefulMaths': '物理中常用的數學',
  // Mechanics translations
  'strand.mechanics.title': '力學',
  'strand.mechanics.desc': '位置和移動、運動方程、力、功、能量和功率、動量、拋體與圓周運動、萬有引力',
  'strand.mechanics.subtitle': '力學 — 力與運動',
  'mechanics.section.empty': '力學單元內容即將推出。',
  'tools.vectorTool.title': '矢量實驗室 — 分解',
  'tools.newtonFirstLaw.title': '牛頓第一定律 — 光滑地面',
  'tools.elevatorWeight.title': '電梯 — 法向力與加速度',
  'tools.stackedBoxes.title': '疊箱 — 牛頓定律與摩擦力',
  'tools.seesawMoment.title': '蹺蹺板 — 力矩原理',
  'tools.ladderMoment.title': '梯子 — 力矩與平衡',
  'tools.supportsMoment.title': '支架 — 力矩與反作用力',
  'tools.impulseMomentum.title': '衝量與動量變化',
  'tools.crashTrackMomentum.title': '碰撞軌道',
  'tools.bulletMomentum.title': '子彈擺 — 彈道擺',
  'tools.projectileMotion.title': '拋體運動',
  'tools.orbitalForces.title': '軌道力實驗室 — 萬有引力與向心力',
  'tools.bungeeEnergy.title': '笨豬跳 — 重力、動能與彈性勢能',
  'tools.divingEnergy.title': '跳水 — 能量與水面',
  'tools.pendulumEnergy.title': '單擺 — 動能與重力勢能',
  'tools.skateParkEnergy.title': '滑板公園 — 能量守恆',
  'tools.aircraftTurn.title': '飛機轉彎 — 傾側與向心力',
  'tools.conicalPendulum.title': '圓錐擺 — 張力與向心力',
  // Gas and kinetic theory translations
  'strand.gas.title': '\u6c23\u9ad4\u8207\u5206\u5b50\u904b\u52d5\u8ad6',
  'strand.gas.desc': '氣體、氣體分子運動論、氣體壓強與分子運動',
  'strand.gas.subtitle': '氣體與分子運動論 — 氣體定律與分子動能',
  'gas.section.empty': '氣體單元內容即將推出。',
  'topic.gasLaws': '氣體',
  'topic.kineticTheory': '氣體分子運動論',
  'topic.gasPressure': '氣體壓強與分子運動',
  'notes.card.gasLaws': 'Ch.5.1 氣體',
  'notes.card.kineticTheory': 'Ch.5.2 氣體分子運動論',
  'notes.card.gasPressure': 'Ch.5.3 氣體壓強與分子運動',
  'summary.item.gasLaws': '氣體',
  'summary.item.kineticTheory': '氣體分子運動論',
  'summary.item.gasPressure': '氣體壓強與分子運動',
  'gas.comingSoon': '此單元的工作紙、測驗及閃卡將於日後推出。',
  'tools.boylesLaw.title': '波義耳定律 — 擠壓注射器',
  'tools.charlesLaw.title': '查理定律 — 體積與開氏溫度',
  'tools.pressureLaw.title': '壓强定律 — 加熱密閉容器',
  'tools.connectedContainers.title': '連通容器 — 打開活栓',
  'tools.kineticTheory.title': '分子動力論 — 分子與氣體定律',
  'topic.positionMovement': '位置和移動',
  'topic.equationOfMotion': '運動方程',
  'topic.verticalMotion': '鉛直運動',
  'topic.motionGraph': '運動圖像',
  'topic.forceI': '力 I',
  'topic.forceII': '力 II',
  'topic.forceIII': '力 III',
  'topic.moment': '力矩',
  'topic.workEnergyPower': '功、能量和功率',
  'topic.momentum': '動量',
  'topic.projectileMotion': '拋體運動',
  'topic.circularMotion': '圓周運動',
  'topic.gravitationalForce': '萬有引力',
  'notes.card.positionMovement': 'Ch.1 位置和移動',
  'notes.card.equationOfMotion': 'Ch.2 運動方程',
  'notes.card.verticalMotion': 'Ch.3 鉛直運動',
  'notes.card.motionGraph': 'Ch.4 運動圖像',
  'notes.card.forceI': 'Ch.5 力 I',
  'notes.card.forceII': 'Ch.6 力 II',
  'notes.card.forceIII': 'Ch.7 力 III',
  'notes.card.moment': 'Ch.8 力矩',
  'notes.card.workEnergyPower': 'Ch.9 功、能量和功率',
  'notes.card.momentum': 'Ch.10 動量',
  'notes.card.projectileMotion': 'Ch.11 拋體運動',
  'notes.card.circularMotion': 'Ch.12 圓周運動',
  'notes.card.gravitationalForce': 'Ch.13 萬有引力',
  'summary.item.positionMovement': '位置和移動',
  'summary.item.equationOfMotion': '運動方程',
  'summary.item.verticalMotion': '鉛直運動',
  'summary.item.motionGraph': '運動圖像',
  'summary.item.forceI': '力 I',
  'summary.item.forceII': '力 II',
  'summary.item.forceIII': '力 III',
  'summary.item.moment': '力矩',
  'summary.item.workEnergyPower': '功、能量和功率',
  'summary.item.momentum': '動量',
  'summary.item.projectileMotion': '拋體運動',
  'summary.item.circularMotion': '圓周運動',
  'summary.item.gravitationalForce': '萬有引力',
  'tools.comingSoon': '力學單元的互動實驗室將於日後推出。',
  'worksheets.comingSoon': '力學單元的工作紙將於日後推出。',
  'flashcards.comingSoon': '力學單元的閃卡將於日後推出。',
  // Waves translations
  'strand.waves.title': '波動',
  'strand.waves.desc': '波動運動、波動特性、駐波、光波、聲波',
  'strand.waves.subtitle': '波動 — 波動運動與波動現象',
  'waves.section.empty': '波動單元內容即將推出。',
  'topic.waveMotion': '波動運動',
  'topic.waveProperties': '波動特性',
  'topic.stationaryWave': '駐波',
  'topic.lightWave': '光波',
  'topic.soundWave': '聲波',
  'notes.card.waveMotion': 'Ch.3.1 波動運動',
  'notes.card.waveProperties': 'Ch.3.2 波動特性',
  'notes.card.stationaryWave': 'Ch.3.3 駐波',
  'notes.card.lightWave': 'Ch.3.4 光波',
  'notes.card.soundWave': 'Ch.3.5 聲波',
  'summary.item.waveMotion': '波動運動',
  'summary.item.waveProperties': '波動特性',
  'summary.item.stationaryWave': '駐波',
  'summary.item.lightWave': '光波',
  'summary.item.soundWave': '聲波',
  'waves.comingSoon': '此單元的互動實驗室、工作紙、測驗及閃卡將於日後推出。',
  // Electricity and magnetism
  'strand.electricity.title': '電和磁',
  'strand.electricity.desc': '靜電、家居電路、電流的磁效應、夫林明手定則、磁場對運動電荷的作用力',
  'strand.electricity.subtitle': '電和磁 — 電荷、電路與磁場',
  'electricity.section.empty': '電和磁單元內容即將推出。',
  'electricity.comingSoon': '此單元的工作紙、測驗及閃卡將於日後推出。',
  'topic.electrostatics': '靜電',
  'topic.electricCircuits': '電路',
  'topic.domesticElectricity': '家居用電',
  'topic.electromagnetism': '電磁現象',
  'topic.electromagneticInduction': '電磁感應',
  'notes.card.electrostatics': '靜電',
  'notes.card.electricCircuits': '電路',
  'notes.card.domesticElectricity': '家居用電',
  'notes.card.electromagnetism': '電磁現象',
  'notes.card.electromagneticInduction': '電磁感應',
  'summary.item.electrostatics': '靜電',
  'summary.item.electricCircuits': '電路',
  'summary.item.domesticElectricity': '家居用電',
  'summary.item.electromagnetism': '電磁現象',
  'summary.item.electromagneticInduction': '電磁感應',
  // Radioactivity and nuclear energy
  'strand.radioactivity.title': '放射現象與核能',
  'strand.radioactivity.desc': '輻射與放射現象、原子模型、核能',
  'strand.radioactivity.subtitle': '放射現象與核能 — 輻射與原子核',
  'radioactivity.section.empty': '放射現象與核能單元內容即將推出。',
  'radioactivity.comingSoon': '此單元的工作紙、測驗及閃卡將於日後推出。',
  'topic.radiationRadioactivity': '輻射與放射現象',
  'topic.atomicModel': '原子模型',
  'topic.nuclearEnergy': '核能',
  'notes.card.radiationRadioactivity': '輻射與放射現象',
  'notes.card.atomicModel': '原子模型',
  'notes.card.nuclearEnergy': '核能',
  'summary.item.radiationRadioactivity': '輻射與放射現象',
  'summary.item.atomicModel': '原子模型',
  'summary.item.nuclearEnergy': '核能',
};
